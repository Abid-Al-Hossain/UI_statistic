...
import { DEFAULT_STATISTIC_STATE } from "./_data/StatisticPresets";
...
import { SECTIONS, type SectionId, type StatisticState, type StudioPreset } from "./types";

export default function Page() {
  const { state, set: setState, undo, redo, reset, canUndo, canRedo } = useHistoryState<StatisticState>(DEFAULT_STATISTIC_STATE);
  const [activeSection, setActiveSection] = useState<SectionId>("presets");
  const [activePresetId, setActivePresetId] = useState<string | null>(null);
  const [downloadName] = useState("statistic-component");
  ...
  const update = <K extends keyof StatisticState>(key: K, value: StatisticState[K]) => { setState((current) => ({ ...current, [key]: value })); setActivePresetId(null); };
  const applyPreset = (preset: StudioPreset) => { setState({ ...DEFAULT_STATISTIC_STATE, ...(preset.state as Partial<StatisticState>) }); setActivePresetId(preset.id); setPreviewResetKey((value) => value + 1); };
...
return <AppShell contentOverflow="hidden"><PlaygroundLayout title="Statistic Studio" headerActions={headerActions} controls={controls} preview={output} /><ContrastGuard /></AppShell>;
}
